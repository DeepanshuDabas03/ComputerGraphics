...
void createCubeObject(unsigned int &program, unsigned int &cube_VAO, unsigned int cubeID)
{
    ...
    {
    case 0:
        red = (timeFactor == 0) ? 1.0f : (timeFactor == 1) ? 0.5f
                                                           : 0.0f;
        green = (timeFactor == 0) ? 0.5f : (timeFactor == 1) ? 0.5f
                                                             : 1.0f;
        blue = (timeFactor == 0) ? 0.5f : (timeFactor == 1) ? 1.0f
                                                            : 0.5f;
        break;
    case 1:
        red = (timeFactor == 0) ? 0.5f : (timeFactor == 1) ? 1.0f
                                                           : 0.5f;
        green = (timeFactor == 0) ? 1.0f : (timeFactor == 1) ? 0.5f
                                                             : 0.5f;
        blue = (timeFactor == 0) ? 0.5f : (timeFactor == 1) ? 0.5f
                                                            : 1.0f;
        break;
    default:
        red = (timeFactor == 0) ? 0.5f : (timeFactor == 1) ? 0.0f
                                                           : 1.0f;
        green = (timeFactor == 0) ? 0.5f : (timeFactor == 1) ? 1.0f
                                                             : 0.5f;
        blue = (timeFactor == 0) ? 1.0f : (timeFactor == 1) ? 0.5f
                                                            : 0.5f;
        break;
    }
    ...
    // ----------------------------------CODE--HERE--------------------------------------
    glUseProgram(program);
    GLint vColor_uniform = glGetUniformLocation(program, "vColor");
    if (vColor_uniform != -1)
    {
        glUniform3f(vColor_uniform, red, green, blue);
    }
    else
    {
        fprintf(stderr, "Could not find uniform location: vColor\n");
        exit(0);
    }
